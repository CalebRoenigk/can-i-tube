...
function selectItem(itemID) {
  ...
  $('.selected-river').empty().text($('#' + itemID).text()).attr('data-index', $('#' + itemID).attr('data-index')); // Close the menu

  $('#dropdown-wrapper').attr('data-open', 'false');
  $('#dropdown-wrapper').height($('.selected-river').height() + 16).width($('.selected-river').text().length + 3 + 'ch');
  $('body').css('background-color', '');
  $('.selected-river').width($('.selected-river').text().length + 3 + 'ch').css('background-color', 'rgba(29, 29, 29, 0)'); // Darken the content-question text

  $('.content-question').css('opacity', '1'); // Drop down the CTA
...
$(document).ready(function () {
  ...
  $('.central-content').css('transform', 'translate(' + -contentWidth / 2 + 'px, ' + -contentHeight / 2 + 'px)'); // Store the width and height values on the object
...
$('.cta-wrapper').click(async function () {
  ...
  selectedRiver = riverInfo[selectedRiverIndex].river; // Name of the real-time cookie for the current site
  ...
  $('.item-list').remove(); // Create the fake loading bar keyframes
  ...
  $('#right-subtitle').css('top', $('#right-content-question').offset().top).css('left', $('#right-content-question').offset().left).css('transform', 'translate(0%, 0%)'); // Position the Title
  ...
  $('#stroke-title').text($('.selected-river').text()).css('top', $('.selected-river').offset().top).css('left', $('.selected-river').offset().left).css('transform', 'translate(0%, 0%)'); // Begin the transition to load state
  // Animate out the dropdown arrow and CTA

  $('.cta-mask > .container').css('transform', 'translateY(125%)');
  $('#dropdown-wrapper').attr('data-open', '').attr('data-animating', 'true'); // Start the call to the USGS after animating to the loading phase
  ...
  $('#fill-title').removeClass('load-bar').addClass('load-bar-finish'); // Begin styling and transitioning the page
...
function generateLoadFrames(time) {
  ...
  } while (percentDone < 100); // This function returns a random integer between a min and max value
...
function fetchRealTimeData(site, cookieName) {
  ...
  if (dataCookiePresent == false) {
    ...
    console.log("Data Already Present!"); // Here, convert the data to the app-ready format

    var appReadyData = getCookie(cookieName); // Return the app-ready data
...
function parseRealTimeJSON(data) {
  var dataFlow = data.value.timeSeries[0].values[0].value[0].value; //var dataDataTime = data.value.timeSeries[0].values[0].value[0].dateTime;
...
function loadingStage() {
  gsap.timeline().set('#title-wrapper', {
    ...
    duration: .6,
    ease: 'power2.inOut',
    left: subtitleSidePosition($('#left-subtitle').width(), $('#fill-title').width(), .625, 2.25, 20),
    scale: .625,
    x: '-50%'
  }, '-=.6').to('#right-subtitle', {
    duration: .6,
    ease: 'power2.inOut',
    right: subtitleSidePosition($('#right-subtitle').width(), $('#fill-title').width(), .625, 2.25, 20),
    scale: .625,
    x: '50%'
  }, '-=.6').to('#fill-title', {
    duration: .6,
...
function formatPage(state, currentFlowValue, heightValue, safeRange, heightRange) {
  ...
  $('#high-range').text(safeRange.max); // Set the range scale and position the low, high, and indicator
  ...
  $('#range-indicator').css('left', indicatorLeft + '%'); // Height content

  $('#low-height').text(heightRange.min);
  $('#high-height').text(heightRange.max); // Set the height scale and position the low, high, and indicator
  ...
  $('#height-indicator').css('left', indicatorHeightLeft + '%'); // Set all point-indicators to the correct color svg

  $('.point-indicator').css('background-image', 'url(assets/img/indicator-arrow-' + state + '.svg)'); // Begin the transition to the final state
  ...
  gsap.timeline().delay(.25).call(createAnswer, [state, currentFlowValue]).set('#answer > .container', {
    ...
    duration: .75,
    ease: 'power2.inOut',
    top: 75 - $('#fill-title').height() * 2.25 * .25,
    color: formatChoices[state].text_color
